...
public class AnisotropicStressTensorInvariantIIIGraph extends AbstractCrossSectionColourCodedGraph {
	/**
	 * Constructor
	 ...
	 */
	public AnisotropicStressTensorInvariantIIIGraph(AbstractDataSetUniqueId dataSetId, MAJFCStackedPanelWithFrame parent) throws HeadlessException {
		super(dataSetId, parent, 50, BackEndAPI.DPS_KEY_ANISOTROPIC_STRESS_TENSOR_ARRAY[0][0], DAStrings.getString(DAStrings.ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRAPH_TITLE), DAStrings.getString(DAStrings.ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRAPH_LEGEND_TEXT), false);
		showInFrame("blah inv III");
	}
	...
	protected double getDatumAt(AbstractDataSetUniqueId dataSetId, DataPointSummaryIndex dpsIndex, int yCoord, int zCoord) throws BackEndAPIException {
		...
		for (int i = 0; i < BackEndAPI.NUMBER_OF_DIMENSIONS; i++) {
			for (int j = 0; j < BackEndAPI.NUMBER_OF_DIMENSIONS; j++) {
				for (int k = 0; k < BackEndAPI.NUMBER_OF_DIMENSIONS; k++) {
					sum += BackEndAPI.getBackEndAPI().getDataPointSummaryDataFieldAtPoint(dataSetId, yCoord, zCoord, BackEndAPI.DPS_KEY_ANISOTROPIC_STRESS_TENSOR_ARRAY[i][j]) * BackEndAPI.getBackEndAPI().getDataPointSummaryDataFieldAtPoint(dataSetId, yCoord, zCoord, BackEndAPI.DPS_KEY_ANISOTROPIC_STRESS_TENSOR_ARRAY[j][k]) * BackEndAPI.getBackEndAPI().getDataPointSummaryDataFieldAtPoint(dataSetId, yCoord, zCoord, BackEndAPI.DPS_KEY_ANISOTROPIC_STRESS_TENSOR_ARRAY[k][i]);				
				}
			}
		}
	...
	protected void showTheVerticalSectionGraph() {
		if (mVerticalSectionGraph == null) {
			String gridTitle = DAStrings.getString(DAStrings.VERTICAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_TITLE);
			String xAxisTitle = DAStrings.getString(DAStrings.VERTICAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_X_AXIS_TITLE);
			String yAxisTitle = DAStrings.getString(DAStrings.VERTICAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_Y_AXIS_TITLE);
			String legendText = DAStrings.getString(DAStrings.VERTICAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_LEGEND_KEY_LABEL);
	...
	protected void showTheHorizontalSectionGraph() {
		if (mHorizontalSectionGraph == null) {
			String gridTitle = DAStrings.getString(DAStrings.HORIZONTAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_TITLE);
			String xAxisTitle = DAStrings.getString(DAStrings.HORIZONTAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_X_AXIS_TITLE);
			String yAxisTitle = DAStrings.getString(DAStrings.HORIZONTAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_Y_AXIS_TITLE);
			String legendText = DAStrings.getString(DAStrings.HORIZONTAL_DISTRIBUTION_OF_ANISOTROPIC_STRESS_TENSOR_INVARIANT_III_GRID_LEGEND_KEY_LABEL);
