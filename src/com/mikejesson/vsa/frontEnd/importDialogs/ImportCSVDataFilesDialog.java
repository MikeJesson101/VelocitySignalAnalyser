...
public class ImportCSVDataFilesDialog extends AbstractImportMultiProbeDataFilesDialog {
	/**
	 * @param parent the parent frame
	 * @param modal is this modal?
	 * @throws BackEndAPIException 
	 */
	public ImportCSVDataFilesDialog(AbstractDataSetUniqueId dataSetId, DataSetType dataSetType, Frame parent, boolean modal) throws BackEndAPIException {
		super(dataSetId, parent, DAStrings.getString(DAStrings.IMPORT_CSV_DATA_FILES_DIALOG_TITLE), modal);
	}
	...
	public void importDataPointDataFromFile(File file, int yCoord, int zCoord, double theta, double phi, double alpha, char delimiter, char decimalSeparator) throws BackEndAPIException {
		if (DATools.isUoBDPMSFile(file)) {
			DAFrame.getBackEndAPI().importDataPointDataFromUoBDPMSFile(mDataSetId, file, getProbeCoords(yCoord, zCoord), theta, phi, alpha, getMainProbeIndex(), getFixedProbeIndex(), getSynchParentProbeIndex(), DAFrame.getFrame().getBackEndAPICallBackAdapter());
		} else {
			DAFrame.getBackEndAPI().importDataPointDataFromCSVFile(mDataSetId, file, getProbeCoords(yCoord, zCoord), theta, phi, alpha, delimiter, decimalSeparator, getMainProbeIndex(), getFixedProbeIndex(), getSynchParentProbeIndex(), DAFrame.getFrame().getBackEndAPICallBackAdapter());
		}
	}
}
