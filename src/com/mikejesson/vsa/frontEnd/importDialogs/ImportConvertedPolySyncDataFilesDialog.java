...
public class ImportConvertedPolySyncDataFilesDialog extends AbstractImportMultiProbeDataFilesDialog {
	/**
	 * @param dataSetId The id of the data set this is for
	 * @param parent the parent frame
	 * @param modal is this modal?
	 * @throws BackEndAPIException 
	 */
	public ImportConvertedPolySyncDataFilesDialog(AbstractDataSetUniqueId dataSetId, Frame parent, boolean modal) throws BackEndAPIException {
		super(dataSetId, parent, DAStrings.getString(DAStrings.IMPORT_MULTIPLE_CONVERTED_POLYSYNC_FILES_DIALOG_TITLE), modal);
	}
	...
	protected boolean isAllowedFile(File file) {
		return DATools.isConvertedPolySyncVelocityFile(file);
	}
	...
	public void importDataPointDataFromFile(File file, int yCoord, int zCoord, double theta, double phi, double alpha, char delimiter, char decimalSeparator) throws BackEndAPIException {
		DAFrame.getBackEndAPI().importDataPointDataFromConvertedPolySyncFile(mDataSetId, file, getProbeCoords(yCoord, zCoord), theta, phi, alpha, getMainProbeIndex(), getFixedProbeIndex(), getSynchParentProbeIndex(), DAFrame.getFrame().getBackEndAPICallBackAdapter());
	}
}
